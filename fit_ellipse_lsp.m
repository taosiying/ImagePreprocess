%% 最小二乘法拟合椭圆
function [xc,yc,a,b,theta] = fit_ellipse_lsp( binary )
...
contours = bwboundaries(binary, 'noholes');
    ...
    % 如果没有轮廓，返回默认值
if isempty(contours)
    xc = 0; yc = 0; a = 0; b = 0; theta = 0;
    return;
end

% 获取最大轮廓（假设外围轮廓点最多）
[~, max_idx] = max(cellfun(@length, contours));
boundary = contours{max_idx};
x = boundary(:,2); % x 坐标
y = boundary(:,1); % y 坐标
...
if (nargin>2) & ~isempty( axis_handle ) & (test>0)
    ...
    a =ellipse_t.a;b = ellipse_t.b;xc = ellipse_t.X0_in;yc =ellipse_t.Y0_in;theta = ellipse_t.phi;
end